...
modifying_code_echo = TRUE # echo code that modifies data?
print_tables = 'hide'
...
# Tables and formulas all obtained from CDC (Centres of Disease Control, United States)
# in turn, some of those tables (especially age less than 24 months) and formulas
# are derived from the WHO (World Health Organization)
# for this reason the data at the age of twenty-four months is not 'continous' (has 'kinks')

# Tables and instructions can be found at CDC https://www.cdc.gov/growthcharts/data_tables.htm
# and WHO http://www.who.int/growthref/who2007_bmi_for_age/en/
# WHO formulas are used for BMI calculations

StatForAge <- list(Female =as.tibble(read.csv('StatForAgeF.csv')),
                   Male = as.tibble(read.csv('StatForAgeM.csv')))
WeightForAge <- list(Female =as.tibble(read.csv('WeightForAgeF.csv')),
                   Male = as.tibble(read.csv('WeightForAgeM.csv')))
BMIForAge <- list(Female =as.tibble(read.csv('BMIForAgeF.csv')),
                   Male = as.tibble(read.csv('BMIForAgeM.csv')))
```
...
``` {r}
# calculate height z-scores (stored in $ZStat)
# and percentiles (stored in $PStat) for age
...
FilteredAgePatientData <- FilteredAgePatientData %>%
  ...
  mutate(StatForAgeRows = mapply(function(months, gender) {
    as.numeric(cut(months, StatForAge[[gender]]$Month, include.lowest = TRUE, right = FALSE))
    },
  ...
  mutate(Lstat = mapply(function(row, gender, months) {
    ...
    l <- l + 
      ((months - StatForAge[[gender]][row,'Month']) / 
         (StatForAge[[gender]][row+1,'Month'] - StatForAge[[gender]][row,'Month'])) *
...
``` {r}
# Height (cm) vs Age (years), Female and Male Charts
...
minmax_rows <- min(FilteredAgePatientData$StatForAgeRows):max(FilteredAgePatientData$StatForAgeRows)
...
``` {r}
# calculate weight z-scores (stored in $Zweight)
# and percentiles (stored in $Pweight) for age

FilteredAgePatientData <- FilteredAgePatientData %>%
  # WeightForAgeRow refers to the row number in the WeightForAge[[Gender]]
  mutate(WeightForAgeRows = mapply(function(months, gender) {
    as.numeric(cut(months, WeightForAge[[gender]]$Month, include.lowest = TRUE, right = FALSE))
    },
    Age.in.months, Gender)) %>%
  mutate(Lweight = mapply(function(row, gender, months) {
    l <- WeightForAge[[gender]][row,'L']
    # interpolate between the 'L' value for the 'month below' and the 'month above'
    l <- l + 
      ((months - WeightForAge[[gender]][row,'Month']) / 
         (WeightForAge[[gender]][row+1,'Month'] - WeightForAge[[gender]][row,'Month'])) *
      (WeightForAge[[gender]][row+1,'L'] - WeightForAge[[gender]][row,'L'])
    return (l)
    },
    WeightForAgeRows, Gender, Age.in.months)) %>%
  mutate(Mweight = mapply(function(row, gender, months) {
    m <- WeightForAge[[gender]][row,'M']
    # interpolate between the 'M' value for the 'month below' and the 'month above'
    m <- m + 
      ((months - WeightForAge[[gender]][row,'Month']) / 
         (WeightForAge[[gender]][row+1,'Month'] - WeightForAge[[gender]][row,'Month'])) *
      (WeightForAge[[gender]][row+1,'M'] - WeightForAge[[gender]][row,'M'])
    return (m)
    },
    WeightForAgeRows, Gender, Age.in.months)) %>%
  mutate(Sweight = mapply(function(row, gender, months) {
    s <- WeightForAge[[gender]][row,'S']
    # interpolate between the 'S' value for the 'month below' and the 'month above'
    s <- s + 
      ((months - WeightForAge[[gender]][row,'Month']) / 
         (WeightForAge[[gender]][row+1,'Month'] - WeightForAge[[gender]][row,'Month'])) *
      (WeightForAge[[gender]][row+1,'S'] - WeightForAge[[gender]][row,'S'])
    return (s)
    },
    WeightForAgeRows, Gender, Age.in.months)) %>%
  # Z-score
  mutate(Zweight = ((Weight/as.numeric(Mweight))^as.numeric(Lweight)-1)/
           (as.numeric(Lweight) * as.numeric(Sweight))) %>%
  # percentile
  mutate(Pweight = pnorm(Zweight))

```

``` {r}
# Weight (kg) vs Age (years), Female and Male Charts

# calculate Loess smoother with uncertainty bounds for recorded heights
mf <- loess(Weight ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$Weight) & FilteredAgePatientData$Gender == 'Female',])
mm <- loess(Weight ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$Weight) & FilteredAgePatientData$Gender == 'Male',])
# minimum/maximum rows of WeightForAge rows required for plotting
minmax_rows <- min(FilteredAgePatientData$WeightForAgeRows):max(FilteredAgePatientData$WeightForAgeRows)

p1 <- FilteredAgePatientData %>%
  filter(!is.na(Weight) & !is.na(Age.in.years)) %>%
  filter(Gender == 'Female') %>%
  plot_ly(x = ~Age.in.years, color = I('red')) %>%
  add_markers(y = ~Weight,
          type = 'scatter', mode = 'markers', name = ~'Female',
          showlegend = FALSE,
          hoverinfo = 'text',
          text = ~paste('Age : ', sprintf('%.1f', Age.in.years),
                        '<br>Weight :', sprintf('%.1f', Weight),
                        '<br>Weight-z : ', sprintf('%.1f', Zweight),
                        '<br>Weight-% : ', sprintf('%.0f', Pweight*100),
                        '<br>ID :',id)) %>%
  add_lines(y = ~fitted(loess(Weight ~ Age.in.years)),
            line = list(color = 'rgba(7,164,181,1)'),
            showlegend = FALSE) %>%
  add_ribbons(data = augment(mf),
              ymin = ~.fitted - 1.96 * .se.fit,
              ...
              fillcolor = 'rgba(7,164,181,0.2)',
              showlegend = FALSE) %>% 
  add_lines(x = ~WeightForAge[['Female']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Female']][minmax_rows,]$P3,
            line = list(color = 'rgba(0,0,0,0.1)', dash = 'dash'),
            name = '3rd centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('3rd centile')) %>%
  add_lines(x = ~WeightForAge[['Female']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Female']][minmax_rows,]$P25,
            line = list(color = 'rgba(0,0,0,0.1)'),
            name = '25th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('25th centile')) %>%
  add_lines(x = ~WeightForAge[['Female']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Female']][minmax_rows,]$P50,
            line = list(color = 'rgba(0,0,0,0.25), width = 5'),
            name = '50th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('50th centile')) %>%
  add_lines(x = ~WeightForAge[['Female']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Female']][minmax_rows,]$P75,
            line = list(color = 'rgba(0,0,0,0.1)'),
            name = '75th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('75th centile')) %>%
  add_lines(x = ~WeightForAge[['Female']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Female']][minmax_rows,]$P97,
            line = list(color = 'rgba(0,0,0,0.1)', dash = 'dash'),
            name = '97th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('97th centile')) %>%
  layout(title = 'Weight (kg) vs Age
         Spur Afrika Clinic, January 2018',
         xaxis = list(title = 'Age in years (Female)'),
         yaxis = list(title = 'Weight (kg)'))
...
p2 <- FilteredAgePatientData %>%
  ...
  add_ribbons(data = augment(mm),
            ...
            fillcolor = 'rgba(7,164,181,0.2)',
            showlegend = FALSE) %>%
  add_lines(x = ~WeightForAge[['Male']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Male']][minmax_rows,]$P3,
            line = list(color = 'rgba(0,0,0,0.1)', dash = 'dash'),
            name = '3rd centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('3rd centile')) %>%
  add_lines(x = ~WeightForAge[['Male']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Male']][minmax_rows,]$P25,
            line = list(color = 'rgba(0,0,0,0.1)'),
            name = '25th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('25th centile')) %>%
  add_lines(x = ~WeightForAge[['Male']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Male']][minmax_rows,]$P50,
            line = list(color = 'rgba(0,0,0,0.25), width = 5'),
            name = '50th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('50th centile')) %>%
  add_lines(x = ~WeightForAge[['Male']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Male']][minmax_rows,]$P75,
            line = list(color = 'rgba(0,0,0,0.1)'),
            name = '75th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
            text = ~paste('75th centile')) %>%
  add_lines(x = ~WeightForAge[['Male']][minmax_rows,]$Month/12,
            y = ~WeightForAge[['Male']][minmax_rows,]$P97,
            line = list(color = 'rgba(0,0,0,0.1)', dash = 'dash'),
            name = '97th centile',
            showlegend = FALSE,
            hoverinfo = 'text',
...
## Weight (z-score) vs Age

``` {r}

# calculate Loess smoother with uncertainty bounds
mf <- loess(Zweight ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$Weight) & FilteredAgePatientData$Gender == 'Female',])
mm <- loess(Zweight ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$Weight) & FilteredAgePatientData$Gender == 'Male',])
...
p1 <- FilteredAgePatientData %>%
  ...
  add_markers(y = ~Zweight,
          ...
          text = ~paste('Age : ', sprintf('%.1f', Age.in.years),
                        ...
                        '<br>Weight-% : ', sprintf('%.0f', Pweight*100),
                        '<br>ID :',id)) %>%
  add_lines(y = ~fitted(loess(Zweight ~ Age.in.years)),
            line = list(color = 'rgba(7,164,181,1)'),
            showlegend = FALSE) %>%
  add_ribbons(data = augment(mf),
              ymin = ~.fitted - 1.96 * .se.fit,
              ...
              fillcolor = 'rgba(7,164,181,0.2)',
              showlegend = FALSE) %>% 
  layout(title = 'Weight (z-score) vs Age
         Spur Afrika Clinic, January 2018',
         xaxis = list(title = 'Age in years (Female)'),
         yaxis = list(title = 'Weight (z-score)'))
...
``` {r}
# calculate BMI z-scores (stored in $Zbmi)
# and percentiles (stored in $Pbmi) for age

FilteredAgePatientData <- FilteredAgePatientData %>%
  # BMIForAgeRow refers to the row number in the WeightForAge[[Gender]]
  mutate(BMIForAgeRows = mapply(function(months, gender) {
    as.numeric(cut(months, BMIForAge[[gender]]$Month, include.lowest = TRUE, right = FALSE))
    },
    Age.in.months, Gender)) %>%
  mutate(Lbmi = as.numeric(mapply(function(row, gender, months) {
    l <- BMIForAge[[gender]][row,'L']
    # interpolate between the 'L' value for the 'month below' and the 'month above'
    l <- l + 
      ((months - BMIForAge[[gender]][row,'Month']) / 
         (BMIForAge[[gender]][row+1,'Month'] -BMIForAge[[gender]][row,'Month'])) *
      (BMIForAge[[gender]][row+1,'L'] - BMIForAge[[gender]][row,'L'])
    return (l)
    },
    BMIForAgeRows, Gender, Age.in.months))) %>%
  mutate(Mbmi = as.numeric(mapply(function(row, gender, months) {
    m <- BMIForAge[[gender]][row,'M']
    # interpolate between the 'M' value for the 'month below' and the 'month above'
    m <- m + 
      ((months - BMIForAge[[gender]][row,'Month']) / 
         (BMIForAge[[gender]][row+1,'Month'] - BMIForAge[[gender]][row,'Month'])) *
      (BMIForAge[[gender]][row+1,'M'] - BMIForAge[[gender]][row,'M'])
    return (m)
    },
    BMIForAgeRows, Gender, Age.in.months))) %>%
  mutate(Sbmi = as.numeric(mapply(function(row, gender, months) {
    s <- BMIForAge[[gender]][row,'S']
    # interpolate between the 'S' value for the 'month below' and the 'month above'
    s <- s + 
      ((months - BMIForAge[[gender]][row,'Month']) / 
         (BMIForAge[[gender]][row+1,'Month'] - BMIForAge[[gender]][row,'Month'])) *
      (BMIForAge[[gender]][row+1,'S'] - BMIForAge[[gender]][row,'S'])
    return (s)
    },
    BMIForAgeRows, Gender, Age.in.months))) %>%
  # Z-score
  # formulas available at WHO http://www.who.int/growthref/who2007_bmi_for_age/en/
  # in link labelled 'Computation of centiles and z-scores'
  mutate(Zbmi = ((bmi/as.numeric(Mbmi))^as.numeric(Lbmi)-1)/
           (as.numeric(Lbmi) * as.numeric(Sbmi))) %>%
  mutate(Zbmi_sd3pos = ifelse(Zbmi > 3, Mbmi*(1+Lbmi*Sbmi*3)^(1/Lbmi), 0)) %>%
  mutate(Zbmi_sd3neg = ifelse(Zbmi < -3, Mbmi*(1+Lbmi*Sbmi* -3)^(1/Lbmi), 0)) %>%
  mutate(Zbmi_sd23pos = 
           ifelse(Zbmi > 3, Mbmi*(1+Lbmi*Sbmi*3)^(1/Lbmi)-Mbmi*(1+Lbmi*Sbmi*2)^(1/Lbmi), 0)) %>%
  mutate(Zbmi_sd23neg =
           ifelse(Zbmi < -3, Mbmi*(1+Lbmi*Sbmi*-2)^(1/Lbmi)-Mbmi*(1+Lbmi*Sbmi* -3)^(1/Lbmi), 0)) %>%
  mutate(Zbmi = ifelse(abs(Zbmi) <= 3, Zbmi,
                       ifelse(Zbmi > 3, 3+(bmi-Zbmi_sd3pos)/Zbmi_sd23pos,
                              -3+(bmi-Zbmi_sd3neg)/Zbmi_sd23neg))) %>%
  # percentile
  mutate(Pbmi = pnorm(Zbmi))
...
mm <- loess(bmi ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$Weight) & FilteredAgePatientData$Gender == 'Male',])
# minimum/maximum rows of WeightForAge rows required for plotting
minmax_rows <- min(FilteredAgePatientData$BMIForAgeRows):max(FilteredAgePatientData$BMIForAgeRows)
...
# calculate Loess smoother with uncertainty bounds
mf <- loess(Zbmi ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$bmi) & FilteredAgePatientData$Gender == 'Female',])
mm <- loess(Zbmi ~ Age.in.years,
            data = FilteredAgePatientData[!is.na(FilteredAgePatientData$Age.in.years) &
              !is.na(FilteredAgePatientData$bmi) & FilteredAgePatientData$Gender == 'Male',])
...
p1 <- FilteredAgePatientData %>%
  ...
  add_markers(y = ~Zbmi,
          ...
          text = ~paste('Age : ', sprintf('%.1f', Age.in.years),
                        ...
                        '<br>BMI-% : ', sprintf('%.0f', Pbmi*100),
                        '<br>ID :',id)) %>%
  add_lines(y = ~fitted(loess(Zbmi ~ Age.in.years)),
            line = list(color = 'rgba(7,164,181,1)'),
            showlegend = FALSE) %>%
  add_ribbons(data = augment(mf),
              ymin = ~.fitted - 1.96 * .se.fit,
              ...
              fillcolor = 'rgba(7,164,181,0.2)',
              showlegend = FALSE) %>% 
  layout(title = 'BMI (z-score) vs Age
         Spur Afrika Clinic, January 2018',
         xaxis = list(title = 'Age in years (Female)'),
         yaxis = list(title = 'BMI (z-score)'))
...
ChildFilteredFindingsData <- FilteredAgePatientData %>%
  # do some thing for FilteredAgePatientData
  # patients aged 18 years or younger, does not include children without specified age but where other evidence suggests they are in school
  mutate(CombinedFindings = Map(append,Findings,Findings2)) %>%
  # Findings were stored in two separate columns
  mutate(Findings = lapply(CombinedFindings,
                           function(Finding) as.data.frame(Finding, stringsAsFactors = FALSE))) %>%
...
ChildFilteredFindingsData[ChildFilteredFindingsData$Finding == 'Caries',]$Finding <- 'Dental decay'
ChildFilteredFindingsData[ChildFilteredFindingsData$Finding == 'Tooth decay',]$Finding <- 'Dental decay'
# ChildFilteredFindingsData[ChildFilteredFindingsData$Finding == 'Difficulty hearing',]$Finding <- 'Hearing loss'
# ChildFilteredFindingsData[ChildFilteredFindingsData$Finding == 'Poor hearing',]$Finding <- 'Hearing loss'
ChildFilteredFindingsData[ChildFilteredFindingsData$Finding == 'Itchy eyes',]$Finding <- 'Eye irritation'
ChildFilteredFindingsData[ChildFilteredFindingsData$Finding == 'Mittleschmerz pain',]$Finding <- 'Mittelschmerz pain'
...
ChildFilteredFindingsData <- ChildFilteredFindingsData %>%
  left_join(FindingCategories, by = 'Finding') %>%
  rename(Finding.Category = Category)
```
...
ChildFilteredFindingAnonymisedData <- 
  select(ChildFilteredFindingsData, -Full.name, -Date.of.birth, -Data.collection.date)

save(ChildFilteredFindingAnonymisedData, file='ChildFilteredData.Rda')

save(StatForAge, file='StatForAge.Rda')
save(WeightForAge, file='WeightForAge.Rda')
save(BMIForAge, file='BMIForAge.Rda')
